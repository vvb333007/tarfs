...
### 3. Что такого особенного она умеет?
  ...
  2. Требования к памяти:
     ...
     дополнительная память (RAM) под имена не выделяется.
  ...
  4. Контроль целостности файлов внутри файловой системы (CRC64/ECMA-182): 
     Контроль целостности файлов достигается встройкой специальной контролькной суммы в каждую запись TAR архива таким образом, чтобы это не влияло на 
  ...
  5. Устойчивость к повреждениям:
     ...
     повредить ее так, чтобы она совсем не работала - практически невозможно. TARFS просто пропускает
     битые\нечитаемые данные и анализирует содержимое TAR архива, чтобы найти в нем не битые данные.
  ...
  6. Скорость и предсказуемость:
     ...
     без блокировок, lockless.
     Для поиска inodes используется бинарный поиск по хэшу имени. Разрешение коллизий - через strcmp.
     inodes отсортированы и по хэшу и по алфавиту

     Операция                          : Время
     ----------------------------------:---------------------------------------
     mount()                           : O(3*N)     три прохода по архиву
