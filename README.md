...
Instead of implementing a full writable filesystem, TARFS provides direct zero-copy access to standard POSIX TAR archives stored in memory-mapped Flash. Files of any size can be accessed with virtually no RAM overhead while remaining compatible with familiar POSIX interfaces and standard TAR tools.

Most files included in firmware never change after the image has been built. HTML pages, CSS, JavaScript, images, certificates, fonts, dictionaries, lookup tables, ML models, and similar resources are typically generated during the build process and are only read at runtime. Using a fully writable filesystem to store such data is often unnecessary.
...
* symbolic links
* hard links
* Windows directory junctions (when present in TAR archives created on Windows)
