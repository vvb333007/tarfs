...
open() использует канонические имена TARFS. 
Поскольку каталоги в индексе TARFS хранятся с завершающим '/', путь к каталогу также 
должен оканчиваться '/'. Если требуется POSIX-совместимое открытие каталога без завершающего 
слеша, используйте opendir().
